// the semi-colon before function invocation is a safety net against concatenated
// scripts and/or other plugins which may not be closed properly.
//========HELPER FUNC ==========================//
!function (e, t, a, i) {
    "use strict"
    function n(t, a) {
        this.element = e(t), this.settings = e.extend({}, c, a), this._defaults = c, this._name = A, this.init()
    }

    var k ='&APPID=20082501b00f38a55e5234059991f87b';
    var g = "http://api.openweathermap.org/data/2.5/weather", o = "http://openweathermap.org/img/w/", s = {
        temp: "",
        wind: "",
        cloud: ""
    }, A = "goodweather", c = {
        q: "London",
        units: "metric",
        search: !1,
        bgPic: !1,
        forecast: !1,
        base64: !0,
        ulAttrs: {"class": "goodweather_gallery_wrapper", id: ""},
        icons: {temp: "", wind: "", cloud: ""},
        cssSetting: [{
            list: "list-style: none",
            textShadow: "text-shadow : 2px 2px 3px black",
            wordWrap: "word-wrap: break-word"
        }]
    }
    e.extend(n.prototype, {
        init: function () {
            function t() {
                e.get("http://ipinfo.io", function (t) {
                    var a = t.loc.split(",")[0], i = t.loc.split(",")[1], g = t.country, o = (t.city, "http://api.openweathermap.org/data/2.5/weather?lat=" + a + "&lon=" + i + "&units=metric" + k)
                    A.append('<div class="error-message"></div>'), e(".error-message").html('<img src="svg-loaders/rings.svg" width="160" alt="loader">'), e.get(o, function (t) {
                        e(".error-message").html("")
                        var o = parseFloat(t.main.temp_min.toFixed(1)), s = parseFloat(t.main.temp_max.toFixed(1)), c = t.weather[0].main, w = (t.wind.speed, t.weather[0].description)
                        n.forecast === !0 && d(a, i), n.bgPic !== !1 && r(c)
                        for (var h in t)console.log(t[h])
                        A.append("<div style='width: 100%'><ul><li style='float: left;margin-right: 5%;'><img class='btn-floating btn-large waves-effect waves-light blue' src='http://openweathermap.org/img/w/" + t.weather[0].icon + ".png'></li><li><h5>" + t.name + " " + g + "</h5></li><li style='margin-left: 5%'><span class='goodweather_viewTemp_min' style='font-size: 1.5em'>" + o + " - </span><span class='goodweather_viewTemp_max' style='font-size: 1.5em'> " + s + "&deg;C </span><li style='text-align: center'><button class='waves-effect waves-light btn goodweather_Degree blue'>&deg;C</button><button class='waves-effect waves-light btn goodweather_Fahrenheit '>&deg;F</button></li><li><span class='btn-flat disabled' > " + w + '</span></li></ul></div><div class="goodWeather_Forecast"></div>'), A.find(e(".goodweather_Degree")).click(function () {
                            A.find(e(".goodweather_viewTemp_min")).html(o), A.find(e(".goodweather_viewTemp_max")).html(" - " + s + "&deg;C")
                        }), A.find(e(".goodweather_Fahrenheit")).on("click", function () {
                            A.find(e(".goodweather_viewTemp_min")).html(parseInt(m.converttDegreeToFahrenheit(o))), A.find(e(".goodweather_viewTemp_max")).html(" - " + parseInt(m.converttDegreeToFahrenheit(s)) + "&deg;F")
                        })
                    }, "jsonp")
                }, "jsonp")
            }

            function a(e, t) {
                var a = t.coord.lat, i = t.coord.lon, n = "<ul class='" + e.ulAttrs["class"] + "' style='" + e.cssSetting[0].list + ";" + e.cssSetting[0].textShadow + "'><h2 style='" + e.cssSetting[0].wordWrap + "'>" + t.name + "</h2><p class='loader'></p><li><img class='btn-floating btn-large waves-effect waves-light blue' src='" + o + t.weather[0].icon + ".png'  alt='icon'/>  <span class='goodweather_viewTemp' style='font-size: 1.5em' >  " + t.main.temp + " &deg;C </span><button class='waves-effect waves-light btn goodweather_Degree blue'>&deg;C</button><button class='waves-effect waves-light btn goodweather_Fahrenheit '>&deg;F</button></li><br><li><img src='data:image/png;base64," + s.wind + "' width='30'  alt='icon'/>  <span >| </span> " + t.wind.speed + " m/s</li><br><li><img src='data:image/png;base64," + s.cloud + "' width='30'  alt='icon'/> <span >| </span> " + t.clouds.all + " %</li></ul>"
                return e.forecast === !0 && d(a, i), e.bgPic !== !1 && r(t.main.temp), n
            }

            var i, n = this.settings, A = this.element, c = '<div class="error-message"></div><div class="weatherInfo"></div></p></h3><br><br><div class="goodWeather_Forecast"></div><form class="my-weather-form"><div class="ui transparent icon input"><input class="country-input" type="text" name="country" placeholder="Search"> <input class="waves-effect waves-light btn grey"  type="submit" name="sub" value="send"> </div></form>'
            n.search === !0 ? (A.prepend(c), A.find(e("form.my-weather-form")).submit(function (t) {
                t.preventDefault()
                var o = A.find(e(".country-input")).val()
                console.log("start!"), A.find(e(".error-message")).html('<img src="svg-loaders/rings.svg" width="160" alt="loader">'), n.q = o, i = e.getJSON(g, n, function (t) {
                    try {
                        A.find(e(".weatherInfo")).html(a(n, t)), A.find(e(".country-input")).val(""), A.find(e(".error-message")).html(""), A.find(e("button.goodweather_Degree")).click(function () {
                            A.find(e(".goodweather_viewTemp")).html(t.main.temp + "&deg;C "), A.find(e(".forecast_temp1")).html(t.main.temp + "&deg;C "), A.find(e(".forecast_temp2")).html(t.main.temp + "&deg;C "), A.find(e(".forecast_temp3")).html(t.main.temp + "&deg;C ")
                        }), A.find(e("button.goodweather_Fahrenheit")).on("click", function () {
                            A.find(e(".goodweather_viewTemp")).html(parseInt(m.converttDegreeToFahrenheit(t.main.temp)) + "&deg;F "), A.find(e(".forecast_temp1")).html(parseInt(m.converttDegreeToFahrenheit(t.main.temp)) + "&deg;F "), A.find(e(".forecast_temp2")).html(parseInt(m.converttDegreeToFahrenheit(t.main.temp)) + "&deg;F "), A.find(e(".forecast_temp3")).html(parseInt(m.converttDegreeToFahrenheit(t.main.temp)) + "&deg;F ")
                        })
                    } catch (i) {
                        console.log("Sorry no match! " + i.message), A.find(e(".error-message")).html('<div class="ui negative message"> Sorry no match!  <br><em><small>try again ;)</small></em></div>'), A.find(e(".weatherInfo")).hide()
                    }
                })
            })) : t()
            var d = function (t, a) {
                var i = "http://api.openweathermap.org/data/2.5/forecast?lat=" + t + "&lon=" + a + "&units=metric", n = "http://openweathermap.org/img/w/"
                A.find(e(".goodWeather_Forecast")).append('<img class="fc_preloader" src="svg-loaders/rings.svg" width="60" alt="loader">'), e.get(i, function (t) {
                    function a(e) {
                        return parseFloat(e.toFixed(1))
                    }

                    var i = t.list, g = e(".goodWeather_Forecast")
                    g.remove("img.fc_preloader")
                    var o = {
                        fcDate1: i[0].dt_txt,
                        fcDate2: i[1].dt_txt,
                        fcDate3: i[2].dt_txt
                    }, s = {
                        fcIcon1: i[0].weather[0].icon,
                        fcIcon2: i[1].weather[0].icon,
                        fcIcon3: i[2].weather[0].icon
                    }, c = {
                        temp1: a(i[0].main.temp),
                        temp2: a(i[1].main.temp),
                        temp3: a(i[2].main.temp)
                    }, d = '<h5><em>Forecast</em></h5><table><thead><tr></tr></thead><tbody><tr><td class="goodWeather_date1"><em>' + o.fcDate1 + '</em></td><td class="goodWeather_icon1"><img class="btn-floating btn-mediun waves-effect waves-light blue"src= ' + n + s.fcIcon1 + '.png></td><td class="goodWeather_temp1"><em class="forecast_temp1">' + c.temp1 + '&deg;C</em></td></tr><tr><td class="goodWeather_date2"><em>' + o.fcDate2 + '</em></td><td class="goodWeather_icon2"><img class="btn-floating btn-mediun waves-effect waves-light blue"src= ' + n + s.fcIcon2 + '.png></td><td class="goodWeather_temp2"><em class="forecast_temp2">' + c.temp2 + '&deg;C</em></td></tr><tr><td class="goodWeather_date3"><em>' + o.fcDate3 + '</em></td><td class="goodWeather_icon3"><em><img class="btn-floating btn-mediun waves-effect waves-light blue"src= ' + n + s.fcIcon3 + '.png></td><td class="goodWeather_temp3"><em class="forecast_temp3">' + c.temp3 + "&deg;C</em></td></tr></tbody></table>"
                    A.find(g).html(d)
                })
            }, r = function (t) {
                var a = {
                    temp: function () {
                        return "Clouds" == t ? "cloud" : "Rain" == t ? "drop" : "sun"
                    }
                }, i = "http://www.splashbase.co/api/v1/images/search?query=", g = a.temp()
                e.get(i + g, function (t) {
                    var a, i
                    for (var g in t) {
                        var o = t[g].length
                        i = Math.floor(Math.random() * (o - 1) + 1), a = t[g][i].url
                    }
                    "body" === n.bgPic ? e("body").css({
                        background: "url(" + a + ") no-repeat center center fixed",
                        "background-size": "cover",
                        "-webkit-background-size": "cover",
                        "-moz-background-size": "cover"
                    }) : "self" === n.bgPic && A.css({
                        background: "url(" + a + ") no-repeat center center fixed",
                        "background-size": "cover",
                        "-webkit-background-size": "cover",
                        "-moz-background-size": "cover"
                    })
                })
            }, m = {
                converttDegreeToFahrenheit: function (e) {
                    var t = 1.8 * e + 32
                    return t.toString()
                }
            }
        }
    }), e.fn[A] = function (t) {
        return this.each(function () {
            e.data(this, "plugin_" + A) || e.data(this, "plugin_" + A, new n(this, t))
        })
    }
}(jQuery, window, document)
